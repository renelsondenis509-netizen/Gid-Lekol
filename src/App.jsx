...
// ==============================
// Gid NS4 – Application complète
// ==============================

import React, { useState, useEffect, useRef, useCallback, createContext, useContext } from "react";
import { QUIZ_DATA } from "./quizData.js"; // À fournir séparément
...
function AuthProvider({ children }) {
  ...
  return (
    <AuthContext.Provider value={{ user, login, logout, restoreSession }}>
      {children}
...
function MenuScreen({ onNavigate, onLogout }) {
  const { user } = useAuth();
  return (
    <div className="fixed inset-0 flex flex-col" style={{ background: "linear-gradient(160deg,#0a0f2e,#0d1b4b)" }}>
      ...
      <div className="px-4 pb-4">
        <button onClick={onLogout} className="w-full py-4 rounded-2xl text-red-400 font-semibold" style={{ background: "#d4002a15", border: "1px solid #d4002a30" }}>
          Dekonekte
        </button>
      </div>
...
function AppContent() {
  ...
  const renderScreen = () => {
    switch (screen) {
      ...
      case "menu":
        return <MenuScreen onNavigate={navigate} onLogout={() => { sessionClear(); setUser(null); setScreen("login"); }} />;
      case "payment":
...
export {
  ...
  getQuizGrades,
  saveQuizGrade,
  useIndexedDB as idbHook, // renommé pour éviter conflit
  LoginScreen,
  ChatScreen,
  QuizScreen,
};
