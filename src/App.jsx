// ==============================
// Gid NS4 – Application complète (corrigée - navigation fonctionnelle)
// ==============================

import React, { useState, useEffect, useRef, useCallback, createContext, useContext } from "react";

// Note: Assurez-vous que ce fichier existe et exporte QUIZ_DATA
...
async function callEdge(payload, timeout = 15000) {
  ...
  try {
    ...
    if (err.name === "AbortError") {
      throw { type: "timeout", message: "Délai de réponse dépassé." };
    }
    throw err;
  }
}
    ...
    return (
      <span
        dangerouslySetInnerHTML={{
          __html: html.replace(/\*\*(.+?)\*\*/g, "<strong>$1</strong>"),
        }}
...
function getQuizGrades(phone) {
  try {
    return JSON.parse(localStorage.getItem(`grades_${phone}`) || "{}");
  } catch {
    return {};
  }
}
...
// ─── INDEXEDDB (encapsulé dans un hook) ──────────────────────────────────────
...
function useIndexedDB() {
  const saveScan = useCallback(async (phone, entry) => {
    try {
      ...
      await new Promise((resolve, reject) => {
        ...
        tx.objectStore(STORE_SCANS).add({ ...entry, phone, id: Date.now() });
  ...
  const getScans = useCallback(async (phone, limit = 50) => {
    try {
      const db = await openDB();
      return await new Promise((resolve, reject) => {
        ...
        const range = IDBKeyRange.only(phone);
  ...
  const fallbackSave = (phone, entry) => {
    try {
...
function AuthProvider({ children }) {
  ...
  const login = async (phone, schoolCode) => {
...
function ExpiryBanner({ daysRemaining }) {
  ...
  return (
    <div
      ...
      style={{ background: isUrgent ? "#d4002a" : "#92400e", color: "white" }}
    >
    ...
    >
      <div style={{ animation: "popIn .6s cubic-bezier(.34,1.56,.64,1) both" }}>
        <div
          ...
          style={{ background: "linear-gradient(135deg,#d4002a,#ff6b35)", boxShadow: "0 0 60px #d4002a55" }}
        >
      ...
      <div className="absolute bottom-12 flex gap-2">
        {[0, 1, 2].map((i) => (
          <div key={i} className="w-2 h-2 rounded-full bg-blue-400" style={{ animation: `pulse 1s ${i * 0.2}s infinite` }} />
        ))}
      </div>
      <style>{`
    ...
    <div className="fixed inset-0 flex flex-col" style={{ background: "linear-gradient(160deg,#0a0f2e,#0d1b4b,#1a0505)" }}>
      <div className="flex-1 flex flex-col items-center justify-center px-6">
        <div
          ...
          style={{ background: "linear-gradient(135deg,#d4002a,#ff6b35)", boxShadow: "0 0 40px #d4002a44" }}
        >
        ...
        <div className="w-full max-w-sm space-y-4">
          ...
          <button
            ...
            style={{
              background: loading ? "#333" : "linear-gradient(135deg,#d4002a,#ff6b35)",
...
function ChatScreen({ onNavigate }) {
  ...
  // Si user n'est pas défini (ne devrait pas arriver à cause de la redirection, mais sécurité)
  if (!user) return null;
  ...
  const sendMessage = useCallback(
    async (retryPayload = null) => {
      ...
      if (!retryPayload) {
        setMessages((prev) => [...prev, payload.userMsg]);
        setInput("");
        setImage(null);
      }
      setApiError(null);
    ...
    [input, image, loading, scans, user, activeSubject, detectSubject, messages, saveScan]
  );
  ...
  const handleImage = async (e) => {
    ...
    reader.onload = async (ev) => {
      setImage(await compressImage(ev.target.result));
    };
      ...
      <div className="flex-1 overflow-y-auto px-3 py-4 space-y-4">
        {messages.map((msg, i) => (
          <div
            ...
            style={{ justifyContent: msg.role === "user" ? "flex-end" : "flex-start", animation: "fadeIn .3s ease both" }}
          >
            {msg.role === "assistant" && (
            ...
            <div className="max-w-xs">
              {msg.image && (
                <img src={msg.image} alt="scan" className="rounded-xl mb-2 max-h-40 object-contain" style={{ border: "1px solid #ffffff20" }} />
              )}
              <div
          ...
          <div className="flex gap-2 items-start">
            ...
            <div className="px-4 py-3 rounded-2xl" style={{ background: "#0f1e4a" }}>
              <div className="flex gap-1.5 items-center">
                {[0, 1, 2].map((i) => (
                  <div key={i} className="w-2 h-2 rounded-full bg-blue-400" style={{ animation: `bounce 1s ${i * 0.2}s infinite` }} />
                ))}
      ...
      <div className="px-3 py-3 border-t" style={{ background: "#0a0f2e", borderColor: "#ffffff10" }}>
        {image && (
          <div className="flex items-center gap-2 mb-2 px-2">
            ...
            <button onClick={() => setImage(null)} className="text-red-400 text-lg" aria-label="Supprimer l'image">
              ✕
            </button>
          </div>
        )}
        <div className="flex gap-2 items-end">
          ...
          <button
            ...
            className="w-10 h-10 rounded-xl flex-shrink-0 flex items-center justify-center"
            style={{
              background: loading || remaining <= 0 ? "#1a1a2e" : "linear-gradient(135deg,#d4002a,#ff6b35)",
            }}
            aria-label="Envoyer"
          >
            <span>✈</span>
      ...
      <div className="fixed inset-0 flex flex-col" style={{ background: "#070d1f" }}>
        ...
        <div className="flex-1 overflow-y-auto px-4 py-4 space-y-3">
          ...
          {availableSubjects.map((sub) => (
            ...
            >
              <span style={{ fontSize: 26 }}>{SUBJECT_ICONS[sub]}</span>
              <div className="flex-1">
          ...
          {Object.keys(QUIZ_DATA)
            .filter((s) => !user.subjects.includes(s))
            .map((sub) => (
              <div
                key={sub}
                className="w-full px-5 py-4 rounded-2xl flex items-center gap-4 opacity-30"
        ...
        <div className="flex-1 overflow-y-auto px-4 py-5 space-y-4">
          ...
          {aiCorrection && (
            <div
              ...
              style={{ background: "#0f1e4a", border: "1px solid #1e3a8a33", animation: "fadeIn .4s ease both" }}
            >
      ...
      <div className="fixed inset-0 flex flex-col" style={{ background: "#070d1f" }}>
        <div className="px-4 py-3 border-b" style={{ background: "#0a0f2e", borderColor: "#ffffff10" }}>
          <div className="flex items-center gap-3 mb-2">
            <button onClick={() => setPhase("select")} className="text-blue-400 text-xl">
              ←
            </button>
          ...
          <div className="flex items-center justify-between">
            <span className="text-blue-500 text-xs">
              Wònn {round} • {totalAnswered} kesyon
            </span>
              ...
              return (
                ...
                >
                  <span
                    ...
                    style={{
                      background: selected !== null && idx === currentQ.answer ? "#22c55e" : selected === idx ? "#ef4444" : "#1e3a8a",
                      color: "white",
                    }}
                  >
                    {["A", "B", "C", "D"][idx]}
          ...
          {selected !== null && (
            <div style={{ animation: "fadeIn .3s ease both" }}>
              ...
              <button
                ...
                style={{
                  background: hearts <= 0 ? "linear-gradient(135deg,#d4002a,#ef4444)" : "linear-gradient(135deg,#1a4fd6,#2563eb)",
                }}
              >
        ...
        <div className="w-full max-w-sm space-y-5" style={{ animation: "popIn .5s cubic-bezier(.34,1.56,.64,1) both" }}>
          ...
          <div className="rounded-3xl px-5 py-5 text-center" style={{ background: mention.bg, border: `2px solid ${mention.border}` }}>
            ...
            <div className="font-black mt-1" style={{ fontSize: 48, color: mention.color, lineHeight: 1 }}>
              {note20}
              <span className="text-xl" style={{ color: mention.color + "99" }}>
                /20
              </span>
            </div>
        ...
        <div className="flex-1 overflow-y-auto px-4 py-6 space-y-4">
          ...
          <div className="rounded-3xl px-5 py-5 text-center" style={{ background: mention.bg, border: `2px solid ${mention.border}` }}>
            ...
            <div className="font-black mt-1" style={{ fontSize: 52, color: mention.color, lineHeight: 1 }}>
              {note20}
              <span className="text-xl font-bold" style={{ color: mention.color + "99" }}>
                /20
              </span>
            </div>
            <div className="text-white font-bold text-lg mt-1">{mention.label}</div>
            <div className="text-blue-300 text-sm mt-1">
              {score}/{totalAnswered} kòrèk • {subject}
            </div>
          </div>
                ...
                >
                  ...
                  <div className="flex-1">
                    <div className="flex items-center gap-2">
                      ...
                      {entry.isMe && (
                        <span className="px-2 py-0.5 rounded-full text-xs font-bold" style={{ background: "#1a4fd6", color: "white" }}>
                          Ou
                        </span>
                      )}
                    </div>
...
function HistoryScreen({ onNavigate }) {
  ...
  useEffect(() => {
    getScans(user.phone).then((data) => setHistory(data)).finally(() => setLoading(false));
  }, [getScans, user.phone]);
                  ...
                  <div className="flex gap-3 p-4">
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-2 mb-1">
                        <span className="px-2 py-0.5 rounded-full text-xs font-bold" style={{ background: "#d4002a22", color: "#ff8080" }}>
                          {h.subject}
                        </span>
  ...
  return (
    <div className="fixed inset-0 flex flex-col" style={{ background: "linear-gradient(160deg,#0a0f2e,#0d1b4b)" }}>
      ...
      <div className="px-4 pb-4">
        <button onClick={logout} className="w-full py-4 rounded-2xl text-red-400 font-semibold" style={{ background: "#d4002a15", border: "1px solid #d4002a30" }}>
          Dekonekte
        </button>
      </div>
...
function DashboardScreen({ onBack, userCode }) {
  ...
  // Protection (optionnelle car peut être accessible sans user ? mais userCode est passé)
  if (!user) return null;
  ...
  return (
    <div className="fixed inset-0 flex flex-col" style={{ background: "linear-gradient(160deg,#0a0f2e,#0d1b4b)" }}>
      <div className="flex items-center gap-3 px-4 py-4 border-b" style={{ borderColor: "#ffffff10" }}>
        ...
        <button className="px-3 py-2 rounded-xl text-xs font-bold text-white" style={{ background: "linear-gradient(135deg,#d4002a,#ff6b35)" }}>
          📄 PDF
        </button>
      </div>
      <div className="flex-1 overflow-y-auto px-4 py-4 space-y-4">
  ...
  return (
    <div className="fixed inset-0 flex flex-col" style={{ background: "linear-gradient(160deg,#0a0f2e,#0d1b4b)" }}>
      ...
      <div className="flex-1 overflow-y-auto px-4 py-6 space-y-4">
        ...
        <button
          onClick={() => window.open("https://wa.me/50900000000?text=Bonjou%2C%20mwen%20vle%20vin%20patnè%20Gid%20NS4.", "_blank")}
          className="w-full py-4 rounded-2xl font-bold text-white flex items-center justify-center gap-3"
          style={{ background: "linear-gradient(135deg,#25d366,#128c7e)" }}
        >
...
function AppContent() {
  ...
  useEffect(() => {
    // Une fois le chargement du contexte terminé
    if (!loading) {
      if (user) {
        // Utilisateur connecté, on va vers le chat après le splash
        const timer = setTimeout(() => setScreen("chat"), 1500);
        return () => clearTimeout(timer);
      } else {
        // Pas d'utilisateur, on va vers login après le splash
  ...
  const navigate = (s) => {
    // Vérification de l'authentification pour les écrans protégés
    const protectedScreens = ["chat", "quiz", "leaderboard", "history", "menu", "dashboard"];
    if (!user && protectedScreens.includes(s)) {
      setScreen("login");
    } else {
      setScreen(s);
    }
  ...
  const renderScreen = () => {
    ...
    switch (screen) {
      case "splash":
        return <SplashScreen onDone={() => setScreen(user ? "chat" : "login")} />;
      case "login":
      ...
      case "payment":
        return <PaymentScreen onBack={() => navigate(user ? "menu" : "login")} />;
      case "dashboard":
        return <DashboardScreen onBack={() => navigate("menu")} userCode={user?.code} />;
      case "partner":
        return <PartnerScreen onBack={() => navigate(user ? "menu" : "login")} />;
      default:
        return null;
    }
  };
...
// ─── EXPORTS NOMMÉS (pour tests éventuels) ───────────────────────────────────
export {
  parseApiError,
  scoreToNote20,
  getMention,
